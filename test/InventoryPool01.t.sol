...
contract InventoryPool01Test is Test, Helper {
    ...
    function setUp () public {
        ...
        usdcInventoryPool = InventoryPool01(inventoryPoolDeployer01.deploy(salt, IERC20(USDC), "nomialUSDC", "nmlUSDC", 1 * 10**5));
        vm.stopPrank();
    
        vm.startPrank(WETH_WHALE);
        WETH_ERC20.approve(address(inventoryPoolDeployer01), MAX_UINT);
        wethInventoryPool = InventoryPool01(inventoryPoolDeployer01.deploy(salt, IERC20(WETH), "nomialWETH", "nmlWETH", 1 * 10**14));
        vm.stopPrank();
